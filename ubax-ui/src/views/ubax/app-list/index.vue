    ...
    <el-card shadow="never" class="dashboard-card">
      ...
      <div class="search-bar">
        <el-input
          ...
          @input="handleSearch"
        />
        <el-select v-model="filterTerminal" placeholder="终端筛选" clearable class="filter-select" @change="handleSearch">
          <el-option label="全部" value="" />
          <el-option label="Windows" value="Windows" />
          <el-option label="Linux" value="Linux" />
        </el-select>
        <el-select v-model="filterPlatform" placeholder="平台筛选" clearable class="filter-select" @change="handleSearch">
          <el-option label="全部" value="" />
        ...
        <el-select v-model="filterStatus" placeholder="状态筛选" clearable class="filter-select" @change="handleSearch">
          <el-option label="全部" value="" />
      ...
      <el-table :data="filteredAppList" style="width: 100%" class="app-table mt-4" stripe>
        ...
        <el-table-column prop="terminal" label="终端类型" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="row.terminal === 'Windows' ? '' : 'info'" size="small" effect="light" round>{{ row.terminal }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="platform" label="平台类型" width="130" align="center">
          <template #default="{ row }">
            <el-tag :type="getPlatformTag(row.platform)" size="small" effect="light" round>{{ row.platform }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="healthStatus" label="健康状态" width="110" align="center">
          <template #default="{ row }">
            <el-tag :type="row.healthStatus === '正常' ? 'success' : 'danger'" size="small" effect="light" round>
              {{ row.healthStatus }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="appDialogVisible" :title="appDialogTitle" width="600px">
      <el-form :model="appForm" label-width="100px">
        ...
        <el-form-item label="客户端描述">
          <el-input v-model="appForm.description" type="textarea" :rows="3" placeholder="请输入客户端描述" />
        </el-form-item>
...
const appList = ref([
  ...
  },
  {
    ...
    healthStatus: '正常',
    dailyEvents: 12450,
    createdAt: '2025-02-20 14:15:30'
  },
  {
    id: 3,
    name: 'UBA-X App',
    appId: 'app_ubax_app_001',
    terminal: 'Linux',
    platform: 'App',
    sdkVersion: 'v1.8.3',
    status: 'active',
  ...
  },
  {
    ...
    healthStatus: '正常',
    dailyEvents: 76543,
    createdAt: '2025-03-10 16:45:20'
  },
  {
    id: 5,
    name: '微信小程序',
    appId: 'app_ubax_mini_001',
    terminal: 'Windows',
    platform: '微信小程序',
    sdkVersion: 'v1.5.0',
    status: 'active',
    healthStatus: '异常',
    dailyEvents: 5680,
    createdAt: '2025-04-05 11:20:15'
  },
  {
    id: 6,
    name: 'H5 测试页',
    appId: 'app_ubax_h5_002',
    terminal: 'Linux',
    platform: 'H5',
    sdkVersion: 'v2.0.5',
    status: 'inactive',
...
const filteredAppList = computed(() => {
  ...
  if (searchText.value) {
    ...
    result = result.filter(a =>
      a.name.toLowerCase().includes(keyword) || a.appId.toLowerCase().includes(keyword)
    )
  }
