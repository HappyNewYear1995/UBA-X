...
public class EventConfigServiceImpl implements EventConfigService {
    ...
    public void updateEventConfig(EventConfigSaveReqVO saveReqVO) {
        ...
        AgentRuleConfigDO existConfig = agentEventConfigMapper.selectById(saveReqVO.getId());
        if (existConfig == null) {
            throw exception(EVENT_CONFIG_NOT_EXISTS);
        }
    ...
    public void deleteEventConfig(Long id) {
        AgentRuleConfigDO existConfig = agentEventConfigMapper.selectById(id);
        if (existConfig == null) {
            throw exception(EVENT_CONFIG_NOT_EXISTS);
        }
    ...
    public EventConfigRespVO getEventConfig(Long id) {
        AgentRuleConfigDO eventConfig = agentEventConfigMapper.selectById(id);
        if (eventConfig == null) {
            throw exception(EVENT_CONFIG_NOT_EXISTS);
        }
    ...
    public PageResult<EventConfigRespVO> getEventConfigPage(EventConfigPageReqVO pageReqVO) {
        PageResult<AgentRuleConfigDO> pageResult = agentEventConfigMapper.selectPage(pageReqVO);
        return EventConfigConvert.INSTANCE.convertPage(pageResult);
    }

    @Override
    public List<EventConfigRespVO> getEnabledEventConfigList() {
        List<AgentRuleConfigDO> configs = agentEventConfigMapper.selectListByEnabled(true);
    ...
    public List<EventConfigRespVO> getEnabledEventConfigListByPosition(String matchPosition) {
        List<AgentRuleConfigDO> configs = agentEventConfigMapper.selectListByMatchPosition(matchPosition);
        return configs.stream()
                .map(this::convertToRespVO)
                .collect(Collectors.toList());
    }
    ...
    public List<EventConfigRespVO> matchEvent(String position, String content) {
        ...
        List<AgentRuleConfigDO> configs = agentEventConfigMapper.selectListByMatchPosition(position);
     ...
     */
    private EventConfigRespVO convertToRespVO(AgentRuleConfigDO config) {
