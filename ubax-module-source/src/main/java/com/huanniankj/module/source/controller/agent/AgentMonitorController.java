...
import com.huanniankj.module.source.service.agent.AgentMonitorService;
...
public class AgentMonitorController {
    ...
    @PermitAll
    public CommonResult<Long> createMetric(@Valid @RequestBody AgentMonitorSaveReqVO saveReqVO) {
        return success(agentMonitorService.createMetric(saveReqVO));
    }
    ...
    @PreAuthorize("@ss.hasPermission('source:monitor:query')")
    public CommonResult<AgentMonitorRespVO> getMetric(@RequestParam("id") Long id) {
        return success(agentMonitorService.getMetric(id));
    }

    @GetMapping("/metric/page")
    @Operation(summary = "获得监控指标分页列表", description = "用于管理后台查看监控指标列表")
    @PreAuthorize("@ss.hasPermission('source:monitor:query')")
    public CommonResult<PageResult<AgentMonitorRespVO>> getMetricPage(@Valid AgentMonitorPageReqVO pageReqVO) {
        return success(agentMonitorService.getMetricPage(pageReqVO));
    }
    ...
    @PermitAll
    public CommonResult<List<AgentMonitorRespVO>> getMetricListByAgentUuidAndType(
            @RequestParam("agentUuid") String agentUuid,
            @RequestParam("metricType") String metricType) {
        return success(agentMonitorService.getMetricListByAgentUuidAndType(agentUuid, metricType));
    }
    ...
    @PermitAll
    public CommonResult<List<AgentMonitorRespVO>> getMetricListByAgentUuidAndTimeRange(
            @RequestParam("agentUuid") String agentUuid,
        ...
        return success(agentMonitorService.getMetricListByAgentUuidAndTimeRange(agentUuid, startTime, endTime));
    }
    ...
    @Parameter(name = "endTime", description = "结束时间", required = true)
    @PermitAll
    public CommonResult<AgentMonitorStatisticsRespVO> getAgentStatistics(
            @RequestParam("agentUuid") String agentUuid,
            @RequestParam("startTime") @DateTimeFormat(pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND) LocalDateTime startTime,
            @RequestParam("endTime") @DateTimeFormat(pattern = FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND) LocalDateTime endTime) {
        return success(agentMonitorService.getAgentStatistics(agentUuid, startTime, endTime));
    }
