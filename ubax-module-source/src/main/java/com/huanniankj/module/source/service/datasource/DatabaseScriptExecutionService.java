...
public interface DatabaseScriptExecutionService {
    ...
    /**
     * 执行参数化 SQL 语句（使用 PreparedStatement）
     *
     * @param databaseId 数据源 ID
     * @param sql        SQL 语句（含 ? 占位符）
     * @param params     参数值列表（按顺序对应占位符）
     * @return 执行结果
     */
    DatabaseSqlExecuteRespVO executeSql(Long databaseId, String sql, List<Object> params);
    ...
    /**
     * 执行参数化视图查询（使用 PreparedStatement）
     *
     * @param databaseId 数据源 ID
     * @param viewName   视图名称
     * @param params     参数值列表（按顺序对应占位符）
     * @return 查询结果
     */
    DatabaseSqlExecuteRespVO executeViewQuery(Long databaseId, String viewName, List<Object> params);
